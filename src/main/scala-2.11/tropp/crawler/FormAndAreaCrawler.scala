...
object FormAndAreaCrawler {
  ...
  private val areasStringPattern = """<select id="SelectedLawForm" [^>]+>((?:<option value="[^"]+">[^<]+</option>)+)</select>""".r
  private val formsStringPattern = """<select id="SelectedAreaOfPublic" [^>]+>((?:<option value="[^"]+">[^<]+</option>)+)</select>""".r
    ...
    Gigahorse.withHttp { http =>
      ...
      val crawledForms = allForms.map { case (formId, formName) =>
        val request = Gigahorse
          ...
          .post(s"action=search&Krs=&Name=&Regon=&Province=&District=&City=&IssueDtStart=&IssueDtEnd=&instanceYear=&SelectedLawForm%5B%5D=$formId&page=&sort=&sortdir=")
      ...
      val crawledAreas = allAreas.map { case (areaId, areaName) =>
        val request = Gigahorse
          .url(CrawlerConfig.oppBaseUrl)
          .post(s"action=search&Krs=&Name=&Regon=&Province=&District=&City=&IssueDtStart=&IssueDtEnd=&instanceYear=&SelectedAreaOfPublic%5B%5D=$areaId&page=&sort=&sortdir=")
